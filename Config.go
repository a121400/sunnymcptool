...
// convertSunnyHeaderToStd 将 SunnyNet 的 Header 转换为标准库的 http.Header
func convertSunnyHeaderToStd(src sunnyhttp.Header) http.Header {
	if src == nil {
		return nil
	}
...
// copyStdHeaderToSunny 将标准库的 http.Header 复制到 SunnyNet 的 Header
func copyStdHeaderToSunny(dst sunnyhttp.Header, src http.Header) {
	// 先清空目标
	for k := range dst {
		delete(dst, k)
	}
...
func RunHTTPRequestScriptCode(Conn SunnyNet.HttpConn) (_Display bool) {
	...
	h.URL = Conn.URL()
	h.Header = convertSunnyHeaderToStd(Conn.GetRequestHeader())
	h.Method = Conn.Method()
	h.Proto = "HTTP/1.1"
	h.Body = Conn.GetRequestBody()
	h.Conn = Conn
	...
	respBody := Conn.GetResponseBody()
	if respBody != nil {
		h.Response.Body = respBody
	} else {
		h.Response.Body = []byte{}
	}
	h.Response.StateCode = Conn.GetResponseCode()
	h.Response.Header = convertSunnyHeaderToStd(Conn.GetResponseHeader())

	// 包装 SetAgent 函数以匹配脚本期望的签名
	setAgentWrapper := func(ProxyUrl string) bool {
		return Conn.SetAgent(ProxyUrl)
	}

	_URL, _Method, _Header, _Body, _Header2, _Body2, _StateCode, Display, _Break := _Call(Conn.Theology(), Conn.Type(), Conn.PID(), h.URL, h.Method, h.Header, h.Body, setAgentWrapper, h.Response.Header, h.Response.Body, h.Response.StateCode, h.Display)
	h.Response.StateCode = _StateCode
	if len(_Header2) > 0 {
		h.Response.Header = _Header2
		// 将响应头复制到 Conn 的响应头
		connRespHeader := Conn.GetResponseHeader()
		if connRespHeader != nil {
			copyStdHeaderToSunny(connRespHeader, _Header2)
		}
		if h.Response.StateCode < 100 {
			h.Response.StateCode = 200
		}
		Conn.SetResponseCode(h.Response.StateCode)
	}
	if len(_Body2) > 0 {
		h.Response.Body = _Body2
		Conn.SetResponseBody(_Body2)
		if h.Response.StateCode < 100 {
			h.Response.StateCode = 200
		}
		Conn.SetResponseCode(h.Response.StateCode)
		connRespHeader := Conn.GetResponseHeader()
		if connRespHeader == nil {
			// 使用 StopRequest 来设置完整响应
			respHeader := make(http.Header)
			respHeader.Set("Server", "Sunny")
			respHeader.Set("Accept-Ranges", "bytes")
			respHeader.Set("Connection", "Close")
			respHeader.Set("Content-Length", strconv.Itoa(len(_Body2)))
			h.Response.Header = respHeader
		} else {
			connRespHeader.Set("Content-Length", strconv.Itoa(len(_Body2)))
			h.Response.Header = convertSunnyHeaderToStd(connRespHeader)
		}
	}
	h.Display = Display
	if _URL != h.URL {
	...
	if _Method != h.Method {
		h.Method = _Method
		// 注意：接口不支持直接修改 Method，此处仅更新本地记录
	}
	if !mapsEqual(_Header, h.Header) {
		h.Header = _Header
		// 将请求头复制到 Conn 的请求头
		connReqHeader := Conn.GetRequestHeader()
		if connReqHeader != nil {
			copyStdHeaderToSunny(connReqHeader, _Header)
		}
	}
	if !bytes.Equal(_Body, h.Body) {
		h.Body = _Body
		Conn.SetRequestBody(_Body)
	}
	if Display {
		h.Way = "HTTP"
		h.PID = CommAnd.GetPidName(Conn.PID())
...
func RunHTTPResponseScriptCode(Conn SunnyNet.HttpConn) (_Return_ bool) {
	...
	URL := Conn.URL()
	Header := convertSunnyHeaderToStd(Conn.GetRequestHeader())
	Method := Conn.Method()
	...
	Body2 := Conn.GetResponseBody()
	if Body2 == nil {
		Body2 = make([]byte, 0)
	}
	Header2 := convertSunnyHeaderToStd(Conn.GetResponseHeader())
	lock.Lock()
	_Call := httpFunc
	lock.Unlock()
	if _Call == nil {
	...
	_, _, _, _, _Header, _Body, _StateCode, _, _Break := _Call(Conn.Theology(), Conn.Type(), Conn.PID(), URL, Method, Header, Body, setAgentWrapper, Header2, Body2, StateCode, true)

	if Conn.GetResponseCode() != _StateCode {
		Conn.SetResponseCode(_StateCode)
	}
	if !mapsEqual(_Header, Header2) {
		...
		connRespHeader := Conn.GetResponseHeader()
		if connRespHeader != nil {
			copyStdHeaderToSunny(connRespHeader, _Header)
		}
	}
	if !bytes.Equal(_Body, Body2) {
		Body2 = _Body
		Conn.SetResponseBody(_Body)
	}
...
func RunHTTPErrorScriptCode(Conn SunnyNet.HttpConn) {
	...
	_Call(Conn.Theology(), Conn.Type(), Conn.PID(), URL, Method, Header, Body, setAgentWrapper, make(http.Header), Body2, -1, true)
}
func RunWebSocketScriptCode(Conn SunnyNet.WsConn) (_Return_ bool) {
	...
	URL := Conn.URL()
	Header := make(http.Header) // WebSocket 接口不支持获取请求头
	Method := Conn.Method()
	Body := Conn.Body()
	lock.Lock()
	_Call := wsFunc
	...
	Body2, Break := _Call(Conn.Theology(), Conn.Type(), Conn.PID(), URL, Method, Header, Conn.MessageType(), Body, Conn.SendToServer, Conn.SendToClient, Conn.Close)
	Conn.SetBody(Body2)
	return Break
}
func RunTcpScriptCode(Conn SunnyNet.TcpConn) (_Return_ bool) {
	...
	_Type := 0
	connType := Conn.Type()
	if connType == public.SunnyNetMsgTypeTCPAboutToConnect {
		_Type = 0
	} else if connType == public.SunnyNetMsgTypeTCPConnectOK {
		_Type = 1
	} else if connType == public.SunnyNetMsgTypeTCPClientSend {
		_Type = 2
	} else if connType == public.SunnyNetMsgTypeTCPClientReceive {
		_Type = 3
	} else if connType == public.SunnyNetMsgTypeTCPClose {
		_Type = 4
	}
	Body := Conn.Body()

	// 包装函数以匹配脚本期望的签名
	setConnectionIPWrapper := func(NewAddress string) bool {
		return Conn.SetNewAddress(NewAddress)
	}
	setAgentWrapper := func(ProxyUrl string) bool {
		return Conn.SetAgent(ProxyUrl)
	}

	Body2, Break := _Call(Conn.Theology(), _Type, Conn.PID(), Body, Conn.LocalAddress(), Conn.RemoteAddress(), setConnectionIPWrapper, setAgentWrapper, Conn.SendToServer, Conn.SendToClient, Conn.Close)
	if _Type == 2 || _Type == 3 {
		Conn.SetBody(Body2)
	}
	return Break
...
func RunUdpScriptCode(Conn SunnyNet.UDPConn) (_Return_ bool) {
	...
	_Type := 0
	connType := Conn.Type()
	if connType == public.SunnyNetUDPTypeClosed {
		_Type = 3
	} else if connType == public.SunnyNetUDPTypeSend {
		_Type = 1
	} else {
		_Type = 2
	}
	Body2, Break := _Call(Conn.Theology(), _Type, Conn.PID(), Conn.Body(), Conn.LocalAddress(), Conn.RemoteAddress())
	Conn.SetBody(Body2)
	return Break
}
