	...
	SunnyNetMode := 0
	{
		if Conn.Type() == public.HttpSendRequest {
			...
			if len(b) > 0 {
				...
				respHeader.Set("Server", "SunnyReplaceRules")
				...
				ReplaceHeader(http.Header(Conn.GetRequestHeader()))
				{
					if Conn.GetRequestHeader() != nil {
						_TmpLock.Lock()
						if DisableCache {
							delete(Conn.GetRequestHeader(), "If-None-Match")
							delete(Conn.GetRequestHeader(), "If-Modified-Since")
						}
						_TmpLock.Unlock()
					}
				}
		...
		} else if Conn.Type() == public.HttpResponseOK {
			{
				if Conn.GetResponseHeader() != nil {
					_TmpLock.Lock()
					if DisableCache {
						Conn.GetResponseHeader()["Cache-Control"] = []string{"no-cache, no-store, must-revalidate"}
						Conn.GetResponseHeader()["Pragma"] = []string{"no-cache"}
						Conn.GetResponseHeader()["Expires"] = []string{"0"}
					}
					_TmpLock.Unlock()
				}
			}
			...
			if Conn.GetResponseHeader() != nil {
				...
				if len(Body) > 0 {
					Encoding := ""
					ar := Conn.GetResponseHeader()["Content-Encoding"]
					if len(ar) > 0 {
						Encoding = ar[0]
					} else {
						ar = Conn.GetResponseHeader()["content-encoding"]
					...
					if isOk {
						delete(Conn.GetResponseHeader(), "content-encoding")
						delete(Conn.GetResponseHeader(), "Content-Encoding")
					}
				}
	...
	if Conn.Type() == public.HttpSendRequest {
		{
			...
			// 重新解析 URL（可能已被脚本修改）
			parsedURL, _ := url.Parse(Conn.URL())
			if parsedURL == nil {
				return
			}
		...
		h.Response.Body = []byte(Conn.Error())
		Insert.Lock()
		isUpdateRequestInfo := currentlySelected == Conn.Theology()
		Insert.Unlock()
		if isUpdateRequestInfo {
...
func WSCallback(Conn SunnyNet.ConnWebSocket) {
	if Conn.Type() == public.WebsocketDisconnect {
		...
		Conn.SetBody(ReplaceBody(Conn.Body()))
	}
	...
	// 解析 URL
	wsURL, _ := url.Parse(Conn.URL())
	wsRawQuery := ""
	wsHost := ""
	if wsURL != nil {
		wsRawQuery = wsURL.RawQuery
		wsHost = wsURL.Host
	}
...
func TcpCallback(Conn SunnyNet.ConnTCP) {
	...
	if Conn.Type() == public.SunnyNetMsgTypeTCPClose {
	...
	} else if Conn.Type() == public.SunnyNetMsgTypeTCPAboutToConnect {
		{
			...
			h.Method = string(Conn.Body())
			_ = RunTcpScriptCode(Conn)
			hm := string(Conn.Body())
			if hm != h.Method {
				Conn.SetNewAddress(hm)
			}
			h.URL = Conn.LocalAddress() + "->" + Conn.RemoteAddress()
			newAddr := HostsRulesAddress(Conn.RemoteAddress())
			Conn.SetNewAddress(newAddr)
			return
		}
		//即将连接
	}
