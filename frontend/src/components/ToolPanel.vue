...
export default {
  ...
  methods: {
    ...
    // 自动打开TCP/UDP分析面板
    openAnalyzerPanelForTCP() {
      // 检查面板是否已打开，避免重复打开
      if (window.UI && !window.UI.AnalyzerPanel) {
        this.$nextTick(() => {
          window.UI.AnalyzerPanel = true
          window.SetUILevel("AnalyzerPanel")
          // 通知分析面板加载数据
          this.$nextTick(() => {
            if (window.vm && window.vm.Analyzer && window.vm.Analyzer.Panel) {
              window.vm.Analyzer.Panel.loadPackets()
            }
          })
        })
      } else if (window.UI && window.UI.AnalyzerPanel) {
        // 面板已打开，只刷新数据
        if (window.vm && window.vm.Analyzer && window.vm.Analyzer.Panel) {
          window.vm.Analyzer.Panel.loadPackets()
        }
