...
func event(command string, args *JSON.SyJson) any {
	...
	case "设置IE代理":
		var ok bool
		if args.GetData("Set") == "true" {
			ok = app.App.SetIEProxy()
		} else {
			ok = app.App.CancelIEProxy()
		}
		if !ok {
	...
	case "init":
		...
		//强制走TCP
		{
			...
			_ = app.App.SetMustTcpRegexp(GlobalConfig.MustTcp.Rules, true)
		}
		//证书选择
		{
	...
	case "主动发送":
		...
		SendBool := false
		if direction == "Server" {
			SendBool = h.TcpConn.SendToServer(_Bytes)
		} else {
			SendBool = h.TcpConn.SendToClient(_Bytes)
		}
			if SendBool {
				CallJs("弹出成功提示", "发送成功")
			} else {
		...
		if Type == "Request" {
			...
			case "Args":
				//修改URL中的参数
				{
					...
					currentURL := h.Conn.URL()
					parsedURL, _ := url.Parse(currentURL)
					if parsedURL != nil {
						parsedURL.RawQuery = RawQuery
						h.Conn.UpdateURL(parsedURL.String())
						h.URL = parsedURL.String()
					}
			...
			case "Cookies":
				//修改Cookies
				{
					...
					h.Conn.GetRequestHeader()["Cookie"] = []string{Cookie}
			...
			case "Raw", "Hex":
				{
					...
					h.Response.Conn.SetResponseCode(h.Response.StateCode)
			...
			case "Cookies":
				//修改Cookies
				{
					...
					h.Response.Conn.GetResponseHeader()["Set-Cookie"] = Cookie
					//h.Response.Header = h.Response.Conn.GetResponseHeader()
	...
	case "设置上游代理":
		...
		if Set {
			return app.App.SetGlobalProxy(code, 0)
		}
		app.App.SetGlobalProxy("socket5://:@", 0)
		return true
	case "保存强制TCP使用规则":
		code := strings.ReplaceAll(args.GetData("Data"), "\\\\", "\\")
		if code == "MustTcp" {
			...
			_ = GlobalConfig.saveToFile()
			_TmpLock.Unlock()
			_ = app.App.SetMustTcpRegexp("ALL.ALL.ALL.ALL", false)
		...
		_ = GlobalConfig.saveToFile()
		_TmpLock.Unlock()
		return app.App.SetMustTcpRegexp(code, true) == nil
	case "创建证书":
	...
	case "启动MCP":
		...
		return map[string]interface{}{"success": true, "port": mcpServer.GetPort()}
	case "停止MCP":
		if mcpServer != nil {
			mcpServer.Stop()
		}
	...
	case "获取MCP状态":
		if mcpServer == nil {
			return map[string]interface{}{"running": false, "port": 29999}
		}
		return map[string]interface{}{"running": mcpServer.IsRunning(), "port": mcpServer.GetPort()}
	...
	case "解密数据包":
		...
		if cryptoAnalyzer == nil {
			return map[string]interface{}{"success": false, "error": "加密分析器初始化失败"}
		}
		dataHex := args.GetData("Data")
		if dataHex == "" {
			return map[string]interface{}{"success": false, "error": "数据不能为空"}
		}
		// 移除空格并解码十六进制
		dataHex = strings.ReplaceAll(dataHex, " ", "")
		data, err := hex.DecodeString(dataHex)
		if err != nil {
			return map[string]interface{}{"success": false, "error": "无效的十六进制数据: " + err.Error()}
		}
		if len(data) == 0 {
			return map[string]interface{}{"success": false, "error": "解码后数据为空"}
		}
		result, parseErr := cryptoAnalyzer.ParsePacket(data)
		if parseErr != nil {
			return map[string]interface{}{"success": false, "error": parseErr.Error()}
		}
	...
	case "解密单个数据包":
		...
		theology := getInt(args.GetData("Theology"))
		packetIndex := getInt(args.GetData("Index"))
		if theology <= 0 {
			return map[string]interface{}{"success": false, "error": "无效的Theology参数"}
		}
		h := HashMap.GetRequest(theology)
		if h == nil {
			return map[string]interface{}{"success": false, "error": "请求不存在或已被删除"}
		}
		if packetIndex < 0 || packetIndex >= len(h.SocketData) {
			return map[string]interface{}{"success": false, "error": fmt.Sprintf("数据包索引 %d 超出范围 (0-%d)", packetIndex, len(h.SocketData)-1)}
		}
		socketData := h.SocketData[packetIndex]
		if socketData == nil || socketData.Body == nil {
			return map[string]interface{}{"success": false, "error": "数据包内容为空"}
		}

		// 解析数据包
		result, parseErr := cryptoAnalyzer.ParsePacket(socketData.Body)
		packetInfo := map[string]interface{}{
			"index":     packetIndex,
			"direction": socketData.Info.Ico,
			"length":    len(socketData.Body),
			"rawHex":    formatHex(socketData.Body),
		}
		if socketData.Info != nil {
			packetInfo["time"] = socketData.Info.Time
		}
		if parseErr != nil {
			packetInfo["error"] = parseErr.Error()
			return map[string]interface{}{"success": true, "packet": packetInfo, "decryptError": parseErr.Error()}
		}
		packetInfo["header"] = result.Header
		packetInfo["payloadHex"] = result.PayloadHex
		packetInfo["decryptedHex"] = result.DecryptedHex
		packetInfo["protobufTree"] = result.ProtobufTree
		return map[string]interface{}{"success": true, "packet": packetInfo}
	...
	case "获取加密配置":
		...
		if config == nil {
			return map[string]interface{}{"success": false, "error": "无法获取加密配置"}
		}
		return map[string]interface{}{"success": true, "config": config}
	...
	case "设置加密配置":
		...
		config := &CryptoConfig{
			...
			AESKey:     args.GetData("AESKey"),
			AESIV:      args.GetData("AESIV"),
			HeaderSize: getInt(args.GetData("HeaderSize")),
		}
		if config.Name == "" {
			config.Name = "custom"
		}
		if config.HeaderSize <= 0 {
			config.HeaderSize = 20 // 默认头部大小
		}
		// 验证AES密钥和IV长度
		if len(config.AESKey) != 16 && len(config.AESKey) != 24 && len(config.AESKey) != 32 {
			return map[string]interface{}{"success": false, "error": fmt.Sprintf("AES密钥长度必须是16、24或32字节，当前长度: %d", len(config.AESKey))}
		}
		if len(config.AESIV) != 16 {
			return map[string]interface{}{"success": false, "error": fmt.Sprintf("AES IV长度必须是16字节，当前长度: %d", len(config.AESIV))}
		}
		cryptoAnalyzer.AddConfig(config)
		cryptoAnalyzer.SetCurrentConfig(config.Name)
		return map[string]interface{}{"success": true, "message": "加密配置已更新", "config": config}
	...
	case "列出加密配置":
		...
		return map[string]interface{}{"success": true, "configs": configs, "current": currentName}
	...
	case "加载消息映射":
		...
		jsonStr := args.GetData("Data")
		if jsonStr == "" {
			return map[string]interface{}{"success": false, "error": "JSON数据不能为空"}
		}
		var msgNames map[int]string
		if unmarshalErr := json.Unmarshal([]byte(jsonStr), &msgNames); unmarshalErr != nil {
			return map[string]interface{}{"success": false, "error": "JSON解析失败: " + unmarshalErr.Error()}
		}
		config := cryptoAnalyzer.GetCurrentConfig()
		if config == nil {
			cryptoAnalyzer.LoadDefaultConfig()
		...
		if config != nil {
			config.MsgNames = msgNames
			return map[string]interface{}{"success": true, "count": len(msgNames)}
		}
		return map[string]interface{}{"success": false, "error": "无法获取当前加密配置"}
	...
	case "获取TCP数据包列表":
		...
		protocol := "tcp"
		method := h.Method
		if strings.Contains(strings.ToLower(method), "udp") {
			protocol = "udp"
		}
		...
		for i, socketData := range h.SocketData {
			if socketData == nil {
				continue
			}
			direction := "recv"
			if socketData.Info != nil && socketData.Info.Ico == "上行" {
				direction = "send"
			}
			...
			if socketData.Info != nil {
				packetInfo["time"] = socketData.Info.Time
				packetInfo["bodyHash"] = socketData.Info.BodyHash
			}

			// Base64编码原始数据
			if socketData.Body != nil {
				packetInfo["rawDataB64"] = base64.StdEncoding.EncodeToString(socketData.Body)
			} else {
				packetInfo["rawDataB64"] = ""
			}
	...
	case "解密TCP流":
		...
		if len(h.SocketData) == 0 {
			return map[string]interface{}{"success": true, "packets": []map[string]interface{}{}, "total": 0, "message": "没有数据包"}
		}
		...
		for i, socketData := range h.SocketData {
			...
			if socketData.Info != nil {
				packetInfo["direction"] = socketData.Info.Ico // "上行" 或 "下行"
				packetInfo["time"] = socketData.Info.Time
			}
			if socketData.Body == nil || len(socketData.Body) == 0 {
				packetInfo["error"] = "数据包内容为空"
				results = append(results, packetInfo)
				continue
			}
			result, parseErr := cryptoAnalyzer.ParsePacket(socketData.Body)
			if parseErr != nil {
				packetInfo["error"] = parseErr.Error()
				packetInfo["raw_hex"] = formatHex(socketData.Body)
			} else {
				packetInfo["header"] = result.Header
				packetInfo["raw_hex"] = result.RawHex
				packetInfo["payload_hex"] = result.PayloadHex
				packetInfo["decrypted_hex"] = result.DecryptedHex
				packetInfo["protobuf_tree"] = result.ProtobufTree
			}
			results = append(results, packetInfo)
		}
		return map[string]interface{}{"success": true, "packets": results, "total": len(results)}
	...
	case "解析Protobuf":
		...
		skip := getInt(args.GetData("Skip"))
		tree := cryptoAnalyzer.ParseProtobuf(data, skip)
		return map[string]interface{}{"success": true, "tree": tree}
